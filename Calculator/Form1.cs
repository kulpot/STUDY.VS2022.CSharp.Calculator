    ...
    public partial class Calculator : Form
    {
        ...
        private void changeLabel(int numPressed)
        {
            if (dec == true)
            {
                ...
                if (decimalCount < 1)   /// for one decimal(calculator scenario)
                {
        ...
        {   /// for one plus and minus(calculator scenario)
            int plusMinus = 0;
            foreach (char d in displayLabel.Text)
            {
                if (d == '-')
                {
                    plusMinus++;
        ...
        private void equalButton_Click(object sender, EventArgs e)
        {
            ...
            displayLabel.Text = result.ToString();// value user calculated 
        }
